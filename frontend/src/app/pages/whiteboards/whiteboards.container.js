...
export class Whiteboards extends React.Component {
  ...
  renameView = async (viewId, newName) => {
    await http.patch(
      `projects/${this.state.project.id}/whiteboards/${
        this.state.currentWhiteboard.id
      }/views/${viewId}`,
      { name: newName }
    );
  ...
  deleteView = async viewId => {
    ...
    this.setState({
      views: [...this.state.views.filter(current => current.id !== viewId)]
    });
  };

  addView = async () => {
    const newView = await http.post(
      `projects/${this.state.project.id}/whiteboards/${
        this.state.currentWhiteboard.id
      }/views`,
      { name: 'New view' }
  ...
  deleteWhiteboard = async whiteboardId => {
    await http.delete(
      `projects/${this.state.project.id}/whiteboards/${whiteboardId}`
    );
    this.setState({
      whiteboards: [
        ...this.state.whiteboards.filter(current => current.id !== whiteboardId)
      ]
    });
    ...
    if (this.state.currentWhiteboard.id === whiteboardId) {
      this.props.history.push(
        `/whiteboards/project/${this.state.project.id}/whiteboard/${
          this.state.whiteboards[0].id
        }`,
        {
  ...
  createNewWhiteboard = async () => {
    const newWhiteboard = await http.post(
      `projects/${this.state.project.id}/whiteboards`
    );
  ...
  renameWhiteboard = async newName => {
    const currentWhiteboard = await http.patch(
      `projects/${this.state.project.id}/whiteboards/${
        this.state.currentWhiteboard.id
      }`,
      { name: newName }
    ...
    this.setState(prevState => {
      return {
        ...
        whiteboards: prevState.whiteboards.map(whiteboard => {
          return whiteboard.id === currentWhiteboard.id
            ? currentWhiteboard
            : whiteboard;
        })
      };
  ...
  renameProject = async newName => {
    const project = await http.patch(`projects/${this.state.project.id}`, {
      name: newName
    });
  ...
  async componentDidMount() {
     ...
     */
    const whiteboardId =
      this.props.match.params.whiteboardId || whiteboards[0].id;
    const currentWhiteboard = whiteboards.filter(
