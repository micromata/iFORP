      ...
      >
        <div className="card-header">
          <h5 className="card-title mb-0">
            <Editable
              text={view.name}
              onChange={newName => handleViewNameChange(view.id, newName)}
            />
          </h5>
        </div>
        <div className="card-body">
          <div className="btn-group" role="group">
            {isTheOnlyView === false && (
              <button
                type="button"
                onClick={event => handleDeleteClick(view.id, event)}
            ...
            {view.hasFile === true ? (
              <Link
                className="btn btn-secondary"
                to={`/views/edit/project/${projectId}/whiteboard/${whiteboardId}/view/${
                  view.id
                }`}
              >
              ...
              <Link
                className="btn btn-secondary"
                to={`/library/project/${projectId}/whiteboard/${whiteboardId}/view/${
                  view.id
                }`}
              >
                <span className="oi oi-pencil" />
            ...
            {view.hasFile === true && (
              <Link
                ...
                to={`/views/show/project/${projectId}/whiteboard/${whiteboardId}/view/${
                  view.id
                }`}
              >
